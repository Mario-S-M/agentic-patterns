...
const QUESTION =
  "¿Cuánto costaría juntos el curso de TypeScript y el de Docker " +
...
async function withOutTools() {
  ...
  console.log("\n ⚠️ Verificar los números contra el catálogo de cursos");

  return tracer.summary();
}

//! =========== Implementar patron TOOL USE
// Tool #1 find courses

const findCourses = tool({
  description:
    "Buscar cursos en el catálogo de DevTalles, por texto, en el título o por nivel. Utilízala siempre antes de responder cualquier pregunta sobre cursos, precios, cantidad de alumnos",
  ...
  execute: async ({ text, level }) => {
    const filteredCourses = COURSE_CATALOG.filter((course) => {
      const matchesText =
        !text ||
        course.title.toLocaleLowerCase().includes(text.toLocaleLowerCase());
...
const calculateTotal = tool({
  description:
    "Calcula el precio total de una lista de cursos aplicando un descuento porcentual. Utilízala siempre para cualquier operación aritmética: no la calcules mentalmente.",
  inputSchema: z.object({
    ids: z
      .array(z.string())
...
async function withTools() {
  ...
  const { text } = await generateText({
    ...
    instructions:
      "Eres un asistente del catálogo de cursos, DevTalles: no inventes precios, duraciones ni nombres de cursos. Consulta siempre con las herramientas disponibles. ",
    onStepEnd: tracer.onStepFinish,
  });
