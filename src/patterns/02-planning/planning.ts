...
const planSchema = z.object({
  steps: z
    .array(
      z.object({
        goal: z.string().describe("Qué resulve este paso, en una frase"),
        reason: z
          .string()
          .describe("¿Por qué es necesario? y ¿Por que va en esta posición?"),
      }),
    )
    .min(2)
    .max(15)
    .describe("Pasos ordenados. Cada paso resulve UNA sola cosa"),
});
...
async function buildPlan(tracer: ReturnType<typeof createTracer>) {
  const { text, output } = await generateText({
    ...
    prompt: MISSION,
    instructions:
      "Eres un estratega. No resuelvas el operativo todavía. Responde en español. Solo descompónlo en pasos ordenados donde cada paso dependa del resultado anterior. ",
    onStepEnd: tracer.onStepFinish,
  });
...
async function withPlanning() {
  console.log("=========== B) Planificación".blue);
  const tracer = createTracer("con-planificación");

  // Fase 1: Planificación y contruir el plan
  ...
  for (const [index, step] of steps.entries()) {
    console.log(`----> Ejecutando paso ${index + 1}: ${step.goal}`.blue);

    const { text } = await generateText({
      model,
      instructions:
        "Resuelve únicamente el paso indicado. No te adelantes a los sigueintes pasos ni des el operativo final.",
      prompt:
        `Misión original: ${MISSION} \n\n ` + findings.length
          ? `Resuleto hasta ahora: ${findings.join("\n---------\n")}\n\n`
  ...
  const { text: finalPlan } = await generateText({
    model,
    instructions: "Integra los resultados parciales en un operativo final. Sé concreto: en el orden de intervención y de prioridades.",
    prompt: `MSIÓN: \n ${MISSION}\n\n`+
    `RRESULTADOS PARCIALES: \n ${findings.join('\n---\n')}`,
    onStepEnd: tracer.onStepFinish,
  });


  console.log('\n\nOperativo Fianl: ',finalPlan.green);
