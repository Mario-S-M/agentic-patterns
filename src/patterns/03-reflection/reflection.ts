...
const REQUIREMENTS = [
  ...
  'Termina con una línea que empiece por "RIESGO:" (sin nregritas, sin viñetas, sin markdown, sin espacios adelante de la palabra "RIESGO")',
];
...
async function naiveReflection() {
  ...
  // La autoevaluación: Sin rúbrica, sin criterios, sin nada.
  const { output: veredict } = await generateText({
    model,
    output: Output.object({ schema: naiveVeredictSchema }),
    prompt: `TEXTO: \n ${draft}`,
    onStepEnd: tracer.onStepFinish,
  });
  ...
  console.log(
    `Veredicto del modelo: ${veredict.isGoodEnough ? "✅ Esta listo" : "❌ Necesita cambios"}`,
  );
...
async function reflectionWithRubric() {
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    const { output: critique } = await generateText({
      ...
      prompt:
        ...
        `\n\n TEXTO A REVISAR ${draft}`,
      instructions:
        "Eres un revisor estricto Verifica el texto UNO POR UNO contra cada requisito de la lista. No asumas que algo está cumplido: búscalo literalmente en el texto. Es mejor marcar de más que de menos.",
      onStepEnd: tracer.onStepFinish,
    });
    ...
    console.log("Faltantes derectados: ".yellow);
    ...
    const { text: revised } = await generateText({
      model,
      instructions:
        "Reescribe el briefing corriegiendo ÚNICAMENTE los puntos señalados. Conserva lo que ya funcionaba. Respeta el límite de 120 palabras",
      prompt:
        `BRIEFING ACTUAL:\n ${draft}\n\n` +
  ...
  console.log(`\n Versión Fnial (tras ${iteration}) ieraciones: `.blue);
  console.log(draft.green);
  console.log("\n Auditoria: ".blue);
  const score = printAudit(auditBriefing(draft));

  return { ...tracer.summary(), score, iteration }; // score e iteraciones
}
